...
def WPGMA(filename):
    ...
    for i in range(1, len(text_matrix)):
        ...
        g.add_node(elements[0], weight=0)
    ...
    while len(matrix) > 1:
        ...
        # Merge species
        i_count = species[i_].count(',') + 1
        j_count = species[j_].count(',') + 1
        matrix.append([((i_count * matrix[i_][k]) + (j_count * matrix[j_][k])) / (i_count + j_count) for k in range(len(species))])
        ...
        weight = matrix[i_][j_] / 2
        i_weight = "%.1f" % (weight - g.node[species_i]['weight'])
        j_weight = "%.1f" % (weight - g.node[species_j]['weight'])
        new_species = '%s,%s' % (species_i, species_j)
        species.append(new_species)
        g.add_edge(new_species, species_i, weight=i_weight)
        g.add_edge(new_species, species_j, weight=j_weight)
        g.node[new_species]['weight'] = weight
        del species[j_]
        del species[i_]
    ...
    plt.savefig('.'.join(filename.split('.')[:-1]) + '_UPGMA.png')
    plt.close()
